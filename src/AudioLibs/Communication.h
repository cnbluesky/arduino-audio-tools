...
namespace audio_tools
{
  ...
  public:
    ...
    /// Starts to send data to the indicated address / port and Starts to receive data from/with the indicated port
    uint8_t begin(uint16_t localPort, IPAddress remoteAddress, uint16_t remotePort)
    {
      connect();

      remote_address_ext = remoteAddress;
      remote_port_ext = remotePort;
      return WiFiUDP::begin(localPort);
    }
  ...
  enum class TransmitRole : uint8_t
  {
    Sender,
    Receiver
  };
